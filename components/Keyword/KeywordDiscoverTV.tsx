...
import { useDiscoverTVInKeyword } from "./hook/useDiscoverTVInKeyword";

interface KeywordDiscoverTVProps {
  keywordName: string;
  keywordId: string;
}

const KeywordDiscoverTV = ({
  keywordName,
  keywordId,
}: KeywordDiscoverTVProps) => {
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } =
    useDiscoverTVInKeyword(keywordId, keywordName);
  ...
  return (
    <div className="w-full h-auto mb-8">
      <div className="flex flex-wrap justify-start items-center ml-7 mt-2 mb-3 gap-3">
        <div className="font-bold text-[23px]">TV Shows with keyword:</div>
        <div className="inline-block px-3 py-1 my-4 text-[18px] border border-gray-400 bg-base-300">
          {decodeURI(keywordName)}
        </div>
      </div>
...
export default KeywordDiscoverTV;
