...
import TrendingTVSerieCard from "./TrendingTVSerieCard";
import Link from "next/link";
import { useLanguageStore } from "@/store/useLanguageStore";

interface TrendingTVSerieListProps {
  timeWindow: "day" | "week";
}

const TrendingTVSerieList: React.FC<TrendingTVSerieListProps> = ({
  timeWindow,
}) => {
  const { language } = useLanguageStore();

  const { isLoading, error, data } = useQuery({
    queryKey: ["getTrendingTVSerieList", language, timeWindow],
    queryFn: () => fetchTrendingTVSeries(language, 1, timeWindow),
    staleTime: Infinity,
  });
...
export default TrendingTVSerieList;
