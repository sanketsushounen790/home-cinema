...
interface TVSerieWatchlistModalItemsProps {
  watchlistIds: string[];
...
const TVSerieWatchlistModalItems = ({
  watchlistIds,
  item,
  itemData,
  onClose,
}: TVSerieWatchlistModalItemsProps) => {
  const {
    mutate: addItem,
  ...
  const handleSubmitRemoveItemToWatchlist = async () => {
    ...
    try {
      await removeItemAsync(`tv_${itemData.id}`);
    } catch (err) {
      console.error(err);
    }
  };
...
export default TVSerieWatchlistModalItems;
