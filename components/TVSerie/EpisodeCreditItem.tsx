...
interface EpisodeItemProps {
  ep: SeasonDetailEpisode;
  tvId: string | number;
}

export function EpisodeCreditItem({ ep, tvId }: EpisodeItemProps) {
  const { theme } = useThemeStore();

  return (
    <Link
      href={`/tv/${tvId}/season/${ep.season_number}/episode/${ep.episode_number}`}
    >
      <div
        className={`${
          theme === "light"
      ...
      >
        {/* TOP SECTION */}
        <div className="flex flex-col md:flex-row gap-4 p-4 hover:bg-neutral-800 transition">
          {/* Thumbnail + Play Icon */}
    ...
    "
      ...
      "
          ...
          <div className="flex flex-col justify-between flex-1">
            ...
            <p className="text-sm text-gray-500 mt-3">{ep.runtime} minutes</p>
          </div>
        </div>
      </div>
    </Link>
