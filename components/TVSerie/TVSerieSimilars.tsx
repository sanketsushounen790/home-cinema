...
interface TVSerieSimilarsProps {
  tvId: string | number;
  tvName: string;
}

export const TVSerieSimilars = ({ tvId, tvName }: TVSerieSimilarsProps) => {
  const { isLoading, error, data } = useQuery({
    queryKey: ["getTVSerieSimilars", tvId],
    queryFn: () => fetchTVSerieSimilars("en-US", tvId, 1),
    staleTime: Infinity,
  });
          ...
          <>
            ...
            <div className="w-full max-w-auto h-[410px] px-6 py-5 flex justify-start items-start gap-7 overflow-x-scroll touch-pan-x scroll-smooth">
              ...
              <div className="w-[200px] h-[300px] flex-shrink-0 flex items-center justify-center rounded-lg shadow-lg bg-base-300">
                <Link href={`/tv/${tvId}-${tvName}/similars`}>
