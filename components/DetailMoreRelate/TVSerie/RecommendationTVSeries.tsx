...
import { useRecommendationTVSeries } from "./hook/useRecommendationTVSeries";
import TVSerieCard from "@/components/TVSerie/TVSerieCard";

interface RecommendationTVSeriesProps {
  tvId: number;
  tvName: string;
}
...
const RecommendationTVSeries = ({
  tvId,
  tvName,
}: RecommendationTVSeriesProps) => {
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } =
    useRecommendationTVSeries({ tvId });
    ...
    <div className="w-full h-auto mb-8">
      <div className="flex flex-wrap justify-start items-center ml-7 mt-2 mb-3 gap-3">
        <div className="font-bold text-[23px]">
          Recommendations for TV Show{" "}
          <span className="font-bold">"{decodeURI(tvName)}"</span>
        </div>
      </div>
...
export default RecommendationTVSeries;
